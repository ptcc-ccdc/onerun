...
echo "The user 'prestashop' should now have access to database prestashop with the password $pass on localhost"
read -p "Pause"

# Edit Prestashop config file to update database user and password
read -p "You have to edit the config file to fix the database. Replace the user from root to prestashop and password ($pass) if the host is not localhost change that too. Press enter to continue."
sudo nano /var/www/html/prestashop/config/settings.inc.php
sudo chmod 400 /var/www/html/prestashop/config/settings.inc.php

# Update MySQL bind address
sudo cp /etc/my.cnf.d/server.cnf /etc/my.cnf.d/server.cnf.bak
echo "[mysqld]" >/etc/my.cnf.d/server.cnf
echo "bind-address = localhost" >>/etc/my.cnf.d/server.cnf
read -p "Verify bind-address = localhost is set in /etc/my.cnf.d/server.cnf (Press enter)"
sudo nano /etc/my.cnf.d/server.cnf
sudo systemctl restart mariadb.service
sudo systemctl status mariadb.service
read -p "Status should be green running"
...
cp index.html /usr/share/phpMyAdmin/
...
echo "Should proably run firewall setup in onerun"
unset pass
read -p "Pause"
